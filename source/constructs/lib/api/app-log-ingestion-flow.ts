...
export class AppIngestionFlowStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: PipelineFlowProps) {
    ...
    const appIngestionFlowFnTask = new tasks.LambdaInvoke(
      this,
      'Update Status',
      {
    ...
    const createS3SourceIngestion = new tasks.LambdaInvoke(
      this,
      'Create S3 as Source ingestion for Light Engine.',
      {
        lambdaFunction:
    ...
    const deleteS3SourceIngestionWhenFailed = new tasks.LambdaInvoke(
      this,
      'Delete ingestion when failed.',
      {
        lambdaFunction:
          props.microBatchStack.microBatchLambdaStack
            .PipelineResourcesBuilderStack.PipelineResourcesBuilder,
    ...
    const deleteS3SourceIngestion = new tasks.LambdaInvoke(
      this,
      'Delete S3 as Source ingestion for Light Engine.',
      {
        lambdaFunction:
          props.microBatchStack.microBatchLambdaStack
            .PipelineResourcesBuilderStack.PipelineResourcesBuilder,
    ...
    const choice = new sfn.Choice(this, 'Choice')
      .when(
        sfn.Condition.and(
          ...
          sfn.Condition.stringEquals(
            '$$.Execution.Input.args.pattern',
            'S3SourceStack'
          )
        ),
        createS3SourceIngestion
      )
      .when(
        sfn.Condition.and(
          sfn.Condition.stringEquals(
        ...
        deleteS3SourceIngestion
      )
      .otherwise(cfnTask);

    cfnTask.next(appIngestionFlowFnTask);
    createS3SourceIngestion.next(appIngestionFlowFnTask);
    createS3SourceIngestion.addCatch(deleteS3SourceIngestionWhenFailed, {
      resultPath: '$.result',
    });
    deleteS3SourceIngestionWhenFailed.next(appIngestionFlowFnTask);
    deleteS3SourceIngestionWhenFailed.addCatch(appIngestionFlowFnTask, {
      resultPath: sfn.JsonPath.DISCARD,
    });
    deleteS3SourceIngestion.next(appIngestionFlowFnTask);
    deleteS3SourceIngestion.addCatch(appIngestionFlowFnTask, {
      resultPath: '$.result',
    });
