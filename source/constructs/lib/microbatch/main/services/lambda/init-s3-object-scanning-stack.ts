...
export class InitLambdaS3ObjectScanningStack extends Construct {
  ...
  constructor(
    scope: Construct,
    id: string,
    props: InitLambdaS3ObjectScanningProps
  ) {
    super(scope, id);
    ...
    const cfnS3ObjectScanningRole = this.S3ObjectScanningRole.node
      .defaultChild as iam.CfnRole;
    cfnS3ObjectScanningRole.overrideLogicalId('S3ObjectScanningRole');

    const S3ObjectScanningRWDDBPolicy = new iam.Policy(
      this,
    ...
    // Override the logical ID
    const cfnS3ObjectScanningRWDDBPolicy = S3ObjectScanningRWDDBPolicy.node
      .defaultChild as iam.CfnPolicy;
    cfnS3ObjectScanningRWDDBPolicy.overrideLogicalId(
      'S3ObjectScanningRWDDBPolicy'
    );

    const S3ObjectScanningRWSQSPolicy = new iam.Policy(
      this,
    ...
    // Override the logical ID
    const cfnS3ObjectScanningRWSQSPolicy = S3ObjectScanningRWSQSPolicy.node
      .defaultChild as iam.CfnPolicy;
    cfnS3ObjectScanningRWSQSPolicy.overrideLogicalId(
      'S3ObjectScanningRWSQSPolicy'
    );

    this.S3ObjectScanningRole.addManagedPolicy(
    ...
    this.S3ObjectScanning = new lambda.Function(this, 'S3ObjectScanning', {
      code: lambda.AssetCode.fromAsset(
        path.join(
          __dirname,
          '../../../../../lambda/microbatch/s3_object_scanning'
        ),
        {
    ...
    // Override the logical ID
    const cfnS3ObjectScanning = this.S3ObjectScanning.node
