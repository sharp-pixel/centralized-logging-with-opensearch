...
export class InitLambdaSendTemplateEmailStack extends Construct {
  ...
  constructor(
    scope: Construct,
    id: string,
    props: InitLambdaSendTemplateEmailProps
  ) {
    super(scope, id);
    ...
    const cfnSendTemplateEmailRole = this.SendTemplateEmailRole.node
      .defaultChild as iam.CfnRole;
    cfnSendTemplateEmailRole.overrideLogicalId('SendTemplateEmailRole');

    const SendTemplateEmailRWDDBPolicy = new iam.Policy(
      this,
    ...
    // Override the logical ID
    const cfnSendTemplateEmailRWDDBPolicy = SendTemplateEmailRWDDBPolicy.node
      .defaultChild as iam.CfnPolicy;
    cfnSendTemplateEmailRWDDBPolicy.overrideLogicalId(
      'SendTemplateEmailRWDDBPolicy'
    );

    this.SendTemplateEmailRole.addManagedPolicy(
    ...
    const SendTemplateEmailPolicy = new iam.Policy(
      this,
    ...
    // Override the logical ID
    const cfnSendTemplateEmailPolicy = SendTemplateEmailPolicy.node
      .defaultChild as iam.CfnPolicy;
    ...
    // Override the logical ID
    const cfnSendTemplateEmail = this.SendTemplateEmail.node
      .defaultChild as lambda.CfnFunction;
    cfnSendTemplateEmail.overrideLogicalId('SendTemplateEmail');

    microBatchSNSStack.SNSReceiveStatesFailedTopic.addSubscription(
      new subscriptions.LambdaSubscription(this.SendTemplateEmail)
    );
  }
}
