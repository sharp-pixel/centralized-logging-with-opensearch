...
class TestLinkAccountHelperTrustAccountExists:
    ...
    def test_link_account_ops_corner_case(self):
        ...
        self.acc_helper.update_role = exception_fn  # type: ignore
        with pytest.raises(
            APIException, match=r"Update centralized cloudwatch role failed"
        ):
            extra = {
                "agentConfDoc": "CrossAccount-FluentBitConfigDownloading-6MPXkhKrK4II",
                "agentInstallDoc": "CrossAccount-FluentBitDocumentInstallation-FgTWXJU7Jj0P",
            ...
            self.acc_helper.create_sub_account_link(account_id="666666666666", **extra)

    def test_generate_assume_role_statement_document_cn(self):
        self.acc_helper.default_region = "cn-north-1"
        assert "aws-cn" in self.acc_helper.generate_assume_role_statement_document(
            "666666666666"
        )
